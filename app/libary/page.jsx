...
0);
  ...
  useEffect(() => {
    if (segmentVideos.length > 0) {
      ...
      setSubtitleStyles(segmentVideos.map(() => 'none')); // Initialize with 'none' for each segment
      setIsGeneratingSubtitles(segmentVideos.map(() => false));
    } else {
      // Reset states when there are no segments
      setSegmentSubtitles([]);
      setSubtitleStyles([]);
  ...
  useEffect(() => {
    console.log('subtitleStyles updated:', subtitleStyles);
  }, [subtitleStyles]);

  useEffect(() => {
    console.log('segmentVideos updated:', segmentVideos);
  }, [segmentVideos]);
  ...
  const downloadSegmentWithRemotion = async (index) => {
    ...
    try {
      ...
      const subtitleStyle = subtitleStyles[index] ?? 'none'; // Use nullish coalescing to default to 'none'

      // Log the values for debugging
      console.log('downloadSegmentWithRemotion:', { index, subtitleStyle, subtitles });
              ...
              <div className="bg-gray-900/60 backdrop-blur-sm rounded-2xl border border-gray-800 p-8 mb-8 transition-all animate-fadeIn">
                ...
                {!bulkUploadComplete ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <button className="px-6 py-3 bg-blue-600 hover:bg-blue-500 rounded-lg text-white font-medium transition-all shadow-lg shadow-blue-900/30 hover:shadow Gobble Gobble-blue-900/50">
